...
socket.on("newUserSignUp",(data) => addUser(data,socket)) //Adds new User to the Database
...
socket.on("checkSession",(data) => sessionLoader.verifyUserSession(data,socket))

socket.on("sessionEndet",(data) => sessionLoader.endSession(data))

socket.on("userAttemptToLogIn",(data) => db.userLogInAttempt(data,socket))
...
async function addUser(data,socket) {
  ...
  if(inputCoreckt != true) return socket.emit("userRegistrationInError",inputCoreckt)
