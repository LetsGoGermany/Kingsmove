...
async function userLogInAttempt(data,socket, callback) {
  ...
  if(result.length === 0) return callback("Email does not exists")
  ...
  return socket.emit("userLoggedInSucess",{user_id: result[0].user_id, _id: sessionID})
}

function correctLoginDetails(data) {
